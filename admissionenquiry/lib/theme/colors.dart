import 'package:flutter/material.dart';

// To change all the themes
class CustomColors {
  static const Color lightPurple = Color(0xFFBB86FA);
  static const Color purple = Color(0xFF6002EE);
  static const Color deepPurple = Color(0xFF3900B1);

  static const Color primaryColor = Color(0xFF6002EE);
  static const Color secondaryColor = Color(0xFF3900B1);
  static const Color accentColor = Color(0xFFBB86FA);
  static const Color grey = Color(0xFF848484);
  static const Color darkGrey = Color(0xFF222222);
  static const Color black = Color(0xFF141414);

  static const Color blackdsd = Color(0xFF141414);
}
